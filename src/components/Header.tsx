...
export default function Header({ onCategoryClick, onWhatsAppClick, onCatalogClick }: HeaderProps) {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-40 w-full border-b border-brand-silver/10 bg-[#050505]/90 backdrop-blur-md">
      ...
      {/* Mobile Drawer Menu */}
      {isMobileMenuOpen && (
        <div id="mobile-menu-overlay" className="fixed inset-0 z-50 flex bg-black/85 backdrop-blur-sm md:hidden animate-fadeIn">
          <div className="w-4/5 max-w-sm bg-[#050505] border-r border-brand-silver/10 p-6 flex flex-col h-full shadow-deep">
            <div className="flex items-center justify-between mb-8 border-b border-brand-silver/10 pb-4">
              <div className="flex items-center space-x-2">
                ...
                <img 
                  src={logoImg} 
                  alt="Fruto Prohibido Logo" 
                  className="h-8 w-8 flex-shrink-0 rounded-full object-cover border border-brand-fuchsia/20 shadow-[0_0_10px_rgba(201,2,83,0.2)]"
                  referrerPolicy="no-referrer"
                />
                <span className="font-serif text-base tracking-[0.15em] text-white uppercase font-bold">Menú</span>
              </div>
              <button 
                id="btn-close-mobile-menu"
                onClick={() => setIsMobileMenuOpen(false)} 
                className="p-1 text-brand-silver hover:text-white"
              >
                <X className="h-6 w-6" />
              </button>
            </div>
            ...
            <nav className="flex flex-col space-y-3.5">
              {menuItems.map((item) => (
                <button
                  ...
                  key={item.id}
                  onClick={() => handleCategoryClick(item.id, item.label)}
                  className="text-left py-3 px-4 rounded-lg text-xs tracking-widest uppercase text-brand-silver/85 hover:text-brand-fuchsia hover:bg-white/5 transition-all border-l-2 border-transparent hover:border-brand-fuchsia"
                >
                  {item.label}
                </button>
              ))}
            ...
            <div className="mt-auto border-t border-brand-silver/10 pt-6 space-y-3.5">
              <button
                ...
                className="w-full flex items-center justify-center gap-2 py-3 rounded-lg border border-brand-fuchsia/50 bg-brand-fuchsia/10 text-white text-xs font-semibold tracking-widest uppercase transition-all duration-300 cursor-pointer"
              >
